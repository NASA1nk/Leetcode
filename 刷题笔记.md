...
1. 对字符串进行预处理(统一字符串类型)
   1. 解决奇偶长度问题（全部都是奇数长度）
   2. 在所有的空隙（包括首尾）插入同样的分隔符`#`
2. 定义回文半径RL
   1. 一个回文串中最右位置的字符到对称轴的距离称为回文半径（定义最左位置也可以，但一般对字符串都是从左往右处理）
   2. `RL[i]`表示以第`i`个字符为对称轴的回文串的回文半径（`r-i+1`）
3. `RL[i]-1`
   1. 正好是原本没有插入分隔符的字符串中，以位置`i`为对称轴的最长回文串的长度（是长度，而不再是半径）
4. 所以利用回文串的对称性，扩展回文串求新字符串的RL即可
   1. 记录所有回文串的RL
      1. 对于每个RL[i]，更新达到最右位置`MaxRight`的回文半径和对应的对称轴`pos`
      2. 保证遍历拓展字符串的i在pos的回文范围内
   2. 对于拓展字符串的i
      1. 找到i关于`pos`的对称位置`2*i-pos`
      2. 如果对称位置`2*i-pos`的回文半径为n，那么根据回文的对称性，i的回文半径也为`n`
      3. 特殊情况
         1. `i+n > MaxRight`，则不能根据对称性计算i的回文半径，但可以知道i的回文半径至少为`MaxRight-i`
         2. 对称位置`2*i-pos`对应的回文半径n达到了左边界，i则并没有达到边界
5. 由RL[i]推导出原字符串对应的下标
   1. index = (i-RL[i])/2
