...
比较算法性能 -> 分析算法运行时间（**最坏情况**）

输入规模 n -> 时间复杂度 `T(n)`

**渐进分析**：忽略T(n)的系数和低阶项，只关注高阶项。
...
void quick_sort(int q[],l,r){
    if(l >= r) return;						//定界 l==r 也行
    int x = q[(l+r)/2], i = l-1, j = r + 1;	//从两侧的+1位置开始，先移动
    while(i < j){
        do i++; while(q[i] < x); 			//保证i前面的数都是小于x
        do j--; while(q[j] > x); 			//保证j后面的数都是大于x
        if(i < j){			
     		int t = q[i];
            q[i] = q[j];					//swap(q[i],q[j]); i,j没相遇的时候交换即可   
            q[j] = t;  
        }
    }
    quick_sort(q,1,j);						//quick_sort(q,l,i-1); 
...
处理待排序数组（不具备局部有序性的特点）：分解至数组长度为1时天然有序
A[1,n] -> A[1,n/2]，A[n/2+1,n]
递归解决子问题得到2个有序数组
合并有序子数组
