...
- 慢指针一次走1步，快指针一次走 2 步，当快指针走完的时候慢指针就来到了链表的中间位置。
...
- 如果链表存在环，快指针最后会追上慢指针
- 慢指针如果走了k，则快指针走了2k。那么差距k就是环的长度的整数倍（因为可能慢指针还没进入环的时候，快指针已经在环中循环n圈了），即**k= n*l**，l为环的长度。
...
1. 对字符串进行预处理，解决奇偶长度问题（全部都是奇数长度）

   2. 在所有的空隙（包括首尾）插入分隔符`#`
   2. 再在开头插入`$`防止越界，字符串的末尾默认有`'\0'`，不用插入

2. 回文半径`RL`
   1. 一个回文串中最右位置的字符到对称轴字符的距离称为回文半径（定义最左位置也可以，但一般对字符串都是从左往右处理）
   2. `RL[i]`表示以第`i`个字符为对称轴的回文串的回文半径（`r-i+1`）

3. 回文串长度`RL[i]-1`
   1. 拓展字符串的最大的`RL[i]-1`正好是原字符串中以位置`i`为对称轴的最长回文串的长度,所以找到最大的`RL[i]`即可

4. 回文串的起始位置

   1. 拓展字符串的最大`RL[i]`对应的`(i-RL[i])/2`就是回文串的起始位置（在开头加上`$`就是处理`(i-RL[i])<0`的情况）

5. 所以利用回文串的对称性求拓展字符串的`RL[i]`

   1. 记录和更新最靠右边的回文串的对称轴`pos`和最右位置`MaxRight`
      1. `MaxRight = pos + RL[pos] - 1`;
   2. 求拓展字符串的所有`RL[i]`
      1. 找到`i`关于`pos`的对称点`j = 2 * pos - i`
         1. 由`pos-j = i-pos`推出`j = 2 * pos - i`
         
      2. 如果`MaxRight > i`
         1. 如果`RL[j] < MaxRight - i`，表示以`j`为中心的回文子串包含在以`pos`为中心的回文子串中。由于`i`和`j`对称，所以以i为中心的回文子串也包含在以`pos`为中心的回文子串中，所以有`RL[i] = RL[j]`
         2. 如果`RL[j] >= MaxRight - i`，表示以`j`为中心的回文子串不一定完全包含在以`pos`为中心的回文子串中。所以有`RL[i] >= MaxRig - i`
            1. 因为`MaxRight + 1`关于i对称对应的字符不知道是否等于`MaxRight + 1`对应的字符
         
         即`RL[i] = min(RL[2*pos-i], MaxRight-i)`
         
      3. 如果`MaxRight < i`

         1. 在回文串的外面，不知道是否回文，所以暂时设定`RL[i] = 1`

         即`RL[i]=MaxRight>i?min(RL[2*pos-i], MaxRight-i):1`

      4. 对`i`进行中心拓展得到`RL[i]`

      5. 无论RL[i]是多少 ，如果发现i对应的回文串的右边位置超过了`MaxRight`，就要更新对称轴`pos`和最右位置`MaxRight`
...
class Solution {
    public String longestPalindrome(String s) {
        ...
        for (int i=1; i<l; i++) {
            ...
            if (i - RL[i] >= 0 && i + RL[i] < t.length) {
                // 中心扩展
                while (t[i - RL[i]] == t[i + RL[i]]) { //计算 p[i] 第二步
                    RL[i]++;
                }
            }
